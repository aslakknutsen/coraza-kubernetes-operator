...
| `logging.timeEncoding`                                | string | `rfc3339nano`                                             | Time encoding for structured logs (`rfc3339`, `rfc3339nano`, `epoch`, `millis`). Use RFC 3339 family for compliance auditing |
| `logging.includePodMetadata`                          | bool   | `true`                                                    | Inject `POD_NAME`/`NODE_NAME` via Downward API. On OpenShift with collector enrichment, set `false` to avoid duplicate fields |
