...
| Variable | Purpose |
|---|---|
| `KIND_CLUSTER_NAME` | Connects to a kind cluster (required for current integration tests) |
...
SecRule ARGS "@contains attack" "id:1,phase:2,deny,status:403"`)
    s.CreateRuleSet(ns, "ruleset", []framework.RuleRef{
...
| `CreateRuleSet(ns, name, rules)` | Create RuleSet with cleanup |
| `CreateEngine(ns, name, opts)` | Create Engine with cleanup |
| `TryCreateRuleSet(ns, name, rules)` | Create RuleSet, return error (for validation tests) |
...
| `cross_namespace_test.go` | [#14](https://github.com/networking-incubator/coraza-kubernetes-operator/issues/14) | Cross-namespace reference rejection |
